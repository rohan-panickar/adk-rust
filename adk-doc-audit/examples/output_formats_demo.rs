...
use adk_doc_audit::{
    AuditReport, AuditReportConfig, AuditIssue, IssueCategory, IssueSeverity,
    ReportGenerator, Recommendation, RecommendationType,
};
...
fn create_sample_report() -> AuditReport {
    ...
    .with_suggestion("Update signature to `create_agent(config: AgentConfig) -> Result<Agent>`".to_string())
    ...
    let version_issue = AuditIssue::new(
        ...
        "Referenced version 0.1.5 but current version is 0.1.9".to_string(),
    )
    .with_line_number(15)
    .with_severity(IssueSeverity::Warning)
    .with_suggestion("Update to version 0.1.9".to_string());
    ...
    let fix_api_rec = Recommendation::new(
        ...
        "Review and update all API references to match current implementation signatures".to_string(),
    )
    .with_priority(1)
