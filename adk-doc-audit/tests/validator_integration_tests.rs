...
use adk_doc_audit::{ExampleValidator, CodeExample, AsyncValidationConfig};
use std::env;
...
async fn test_validator_integration() {
    ...
    // Create validator
    let validator = ExampleValidator::new("0.1.0".to_string(), temp_workspace)
        .await
        .unwrap();
...
"#.to_string(),
    ...
    assert!(result.suggestions.iter().any(|s| s.contains("tokio::main")), "Should suggest tokio::main");
}
