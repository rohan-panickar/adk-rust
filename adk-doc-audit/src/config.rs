    ...
    pub fn build(self) -> Result<AuditConfig> {
        ...
        if !config.docs_path.exists() {
            return Err(AuditError::ConfigurationError {
                message: format!("Documentation path does not exist: {}", config.docs_path.display()),
            });
        }
...
impl AuditConfig {
    ...
    pub fn get_database_path(&self) -> PathBuf {
        self.database_path
            .clone()
            .unwrap_or_else(|| self.workspace_path.join(".adk-doc-audit.db"))
    }
}
...
mod tests {
    ...
    fn test_config_validation_errors() {
        ...
        // Test zero timeout
        let result = AuditConfig::builder()
            .example_timeout(Duration::from_secs(0))
            .build();
        assert!(result.is_err());
    }
